...
contract('SmartWalletSwapImplementation', (accounts) => {
  describe('test some simple trades', async () => {
    ...
    it('should be able to repay borrowed ETH using USDT from AAVE v1', async () => {
      const depositToken = {symbol: 'USDT', address: usdtAddress};
      const aEthToken = await IERC20Ext.at(AAVE_V1_ADDRESSES.aEthAddress);
      const usdtToken = await IERC20Ext.at(usdtAddress);
      ...
      const payAmount = new BN(11).mul(new BN(10).pow(new BN(ethDecimals - 1)));
      ...
      tx = await swapProxy.swapUniswapAndRepay(
        ...
        uniswapRouter,
        srcAmount,
        payAmount, // set payAmount to 2x of borrowed, so it's over original payAmount
        tradePath,
        8,
        user,
        false,
    ...
    it('should be able to take fee and deposit ETH to AAVE v2', async () => {
      ...
      let aEthToken = await IERC20Ext.at(AAVE_V2_ADDRESSES.aEthAddress);
      const stakedAmount = srcAmount.sub(srcAmount.mul(new BN(bps)).div(new BN(10000)));
    ...
    // it.only('should be able to repay borrowed ETH using USDT from AAVE v2', async () => {
    //   const depositToken = {symbol: 'USDT', address: usdtAddress};
    //   const aEthToken = await IERC20Ext.at(AAVE_V2_ADDRESSES.aEthAddress);
    //   const usdtToken = await IERC20Ext.at(usdtAddress);
    //   const depositAmount = new BN(5).mul(new BN(10).pow(new BN(ethDecimals)));
    //   const srcAmount = new BN(2000).mul(new BN(10).pow(new BN(await usdtToken.decimals())));
    //   const borrowAmount = new BN(1).mul(new BN(10).pow(new BN(ethDecimals)));
    ...
    //   let tx = await poolV2.deposit(ethAddress, depositAmount, user, 0, {from: user, value: depositAmount});
    //   assertTxSuccess(tx);

    //   tx = await poolV2.borrow(ethAddress, borrowAmount, 2, 0, user, {from: user});
    //   assertTxSuccess(tx);
    ...
    //   const debt = await providerV2.getUserReserveData(ethAddress, user);
    //   assertEqual(debt.currentBorrowBalance, 0, "whole debt not repaid");
    // });
